...
function wptouch_core_header_enqueue() {
	$version = get_bloginfo('version'); 
		...
		wp_enqueue_script('wptouch-core', '' . get_template_directory_uri() . '/core/core.js', array('jquery'),'1.9' );		
		wp_head(); 
		...
		echo "<script src='" . get_stylesheet_directory_uri() . "/core/core.js' type='text/javascript' charset='utf-8'></script>\n"; 
		 }
	}
...
function wptouch_core_header_pages() {
	...
	global $blog_id;
	foreach ($pages as $p) {
		if ( file_exists( wptouch_dir() . '/images/icon-pool/' . $p['icon'] ) ) {
			$image = get_stylesheet_directory_uri() . '/images/icon-pool/' . $p['icon'];	
		}
...
function wptouch_core_header_rss() {
	if (bnc_is_rss_enabled()) {
		echo sprintf(__( "%sRSS Feed%s", "wptouch" ), '<li><a href="' . get_bloginfo('rss2_url') . '"><img src="' . get_stylesheet_directory_uri() . '/images/icon-pool/RSS.png" alt="" />','</a></li>');
	}
}

function wptouch_core_header_email() {
	if (bnc_is_email_enabled()) {
		echo sprintf(__( "%sE-Mail%s", "wptouch" ), '<li><a href="mailto:' . get_bloginfo('admin_email') . '"><img src="' . get_stylesheet_directory_uri() . '/images/icon-pool/Mail.png" alt="" />','</a></li>');
	}
} 
...
function wptouch_core_else_text() {	
	 global $is_ajax; if (($is_ajax) && !is_search()) {
		...
		echo '' . __( "No more search results to display.", "wptouch" ) . '';
	 	...
	 	echo '<div style="padding-bottom:127px">' . __( "No search results found.", "wptouch" ) . '<br />' . __( "Try another query.", "wptouch" ) . '</div>';
	 } else {
	  echo '<div class="post">
...
echo '<script type="text/javascript">function switch_delayer() { window.location = "' . get_bloginfo('url') . '/?ak_action=reject_mobile"}</script>';
echo '' . __( "Mobile Site", "wptouch" ) . ' <a id="switch-link" onclick="wptouch_switch_confirmation(); return false;" href="#"></a>';
}

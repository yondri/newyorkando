...
<html xmlns="http://www.w3.org/1999/xhtml" <?php language_attributes(); ?>>
	<head profile="http://gmpg.org/xfn/11">
		...
		<?php
			if ( is_single() )
				{ single_post_title(); _e(' | ', 'uti_theme'); bloginfo('uti_theme');}
			elseif ( is_home() || is_front_page() )
				{ bloginfo('name'); _e(' | ', 'uti_theme'); bloginfo('description'); }
			elseif ( is_page() )
				{ single_post_title(''); _e(' | ', 'uti_theme'); bloginfo('uti_theme'); }
			elseif ( is_search() )
				{ bloginfo('name'); print __(' | Search results for ', 'uti_theme') . get_search_query(); }
			elseif ( is_404() )
				{ bloginfo('name'); _e(' | Not Found', 'uti_theme'); }
			else
				{ bloginfo('name'); wp_title(__(' | ', 'uti_theme')); }
		?>
		</title>

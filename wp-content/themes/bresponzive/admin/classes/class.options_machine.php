 ...
 * @author      Syamil MJ
 */
class bresponZive_themepacific_Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		$bresponZive_tpcrn_data = $smof_data;
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($smof_data) || !empty($bresponZive_tpcrn_data)){
			...
			switch ( $value['type'] ) {
				...
				case 'slider':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= bresponZive_themepacific_Options_Machine::optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
				...
				case 'sorter':
				    ...
				    $sortlists = isset($bresponZive_tpcrn_data[$value['id']]) && !empty($bresponZive_tpcrn_data[$value['id']]) ? $bresponZive_tpcrn_data[$value['id']] : $value['std'];
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','bresponZive').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case "media":
					...
					$output .= bresponZive_themepacific_Options_Machine::optionsframework_media_uploader_function($value['id'],$u_val, $value['mod']);

...
<?php is_tag(); ?>
	<?php if (have_posts()) : ?>
		...
		<?php if (is_category()) { ?>				
		<h2><?php _e('Archive for '); single_cat_title(); ?></h2>
...
<?php /* If this is a tag archive */ } elseif( is_tag() ) { ?>
		...
		<?php while (have_posts()) : the_post(); ?>
			...
			<div <?php post_class(); ?>>
				...
				<h2 class="posttitle" id="post-<?php the_ID(); ?>"><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php echo esc_attr( sprintf( __( 'Permanent link to %s' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a></h2>
			
				<p class="postmeta"> 
				{ <?php the_time(get_option('date_format')) ?> @ <a href="<?php the_permalink() ?>" rel="bookmark" title="<?php echo esc_attr( sprintf( __( 'Permanent link to %s' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_time() ?></a> }
				&#183; 
				{ <?php the_category(', ') ?> }
				<br />
				<?php the_tags('{ Tags: ', ', ', ' } &#183;'); ?>

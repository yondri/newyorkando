...
<?php the_post() ?>
			...
			<h2 class="page-title"><a href="<?php echo get_permalink($post->post_parent) ?>" title="<?php echo esc_attr( sprintf( __( 'Return to %s', 'sandbox' ), get_the_title( $post->post_parent ) ) ); ?>" rev="attachment"><?php echo get_the_title($post->post_parent) ?></a></h2>

			<div id="post-<?php the_ID() ?>" class="<?php sandbox_post_class() ?>">
				<h3 class="entry-title"><?php the_title() ?></h3>
				<div class="entry-content">
					<div class="entry-attachment"><a href="<?php echo wp_get_attachment_url($post->ID); ?>" title="<?php echo esc_attr( get_the_title( $post->ID ) ); ?>" rel="attachment"><?php echo wp_get_attachment_image( $post->ID, 'auto' ); ?></a></div>

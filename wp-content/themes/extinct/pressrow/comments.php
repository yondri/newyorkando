...
<?php 
	...
	<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="comment_form">
	
		<?php if ( $user_ID ) : ?>
		
			<p><?php printf(__('Logged in as <a href="%1$s">%2$s</a>.'), site_url() . '/wp-admin/profile.php', $user_identity); ?> <a href="<?php echo site_url(); ?>/wp-login.php?action=logout" title="<?php _e('Log out of this account', 'kubrick'); ?>"><?php _e('Logout &raquo;'); ?></a></p>
		...
		<?php else : ?>
			...
			<p><input type="text" class="text_input" name="email" id="email" value="<?php echo $comment_author_email; ?>" tabindex="2" /><label for="email"><?php _e('E-mail'); ?> <?php if ($req) _e("(required)"); ?></label></p>

...
function sandbox_skipnav() {
	...
	echo '<p class="access"><a href="#content" title="'.__('Skip navigation to the content', 'chaostheory').'">'.__('Skip navigation', 'chaostheory').'</a></p>';
}
...
<?php if ( !is_home() || is_paged() ) { ?>
		<?php echo $before_widget; ?>
			<?php echo $before_title ?><a href="<?php bloginfo('url') ?>" title="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>"><?php echo $title ?></a><?php echo $after_title ?>
		<?php echo $after_widget; ?>
<?php } ?>
<?php
}
...
function sandbox_skin_info($skin) {
	$info = array(
		...
		'description' => '',
		'version' => '1.0',
		'author' => __('Anonymous', 'chaostheory'),
		'author_uri' => '',
		'global_navigation' => 'Y',
		);
...
<div class="wrap">
<h2><?php _e('Current Skin', 'chaostheory') ?></h2>
<div id="currenttheme">
<?php if ( file_exists(get_template_directory() . "/skins/$current_skin.png") ) : ?>
<img src="<?php echo get_template_directory_uri() . "/skins/$current_skin.png"; ?>" alt="<?php _e('Current skin preview', 'chaostheory'); ?>" />
<?php endif; ?>
<?php
...
<h2><?php _e('Available Skins', 'chaostheory') ?></h2>
<?php
...
<h2><?php _e('Sandbox Info', 'chaostheory'); ?></h2>
<p><?php printf(__('Check the <a href="%1$s" title="Read the Sandbox readme.html">documentation</a> for help installing new skins and information on the rich semantic markup that makes the Sandbox unique.', 'chaostheory'), get_template_directory_uri() . '/readme.html'); ?></p>
</div>
<?php
}
...
function sandbox_init() {
	load_theme_textdomain('chaostheory');
...
function sandbox_admin_menu() {
	add_theme_page(__('Theme Options', 'chaostheory'), __('Theme Options', 'chaostheory'), 'edit_theme_options', 'skins', 'sandbox_admin_skins');
}
...
function sandbox_widgets_init() {
	...
	wp_register_sidebar_widget('search', __('Search', 'chaostheory'), 'widget_sandbox_search');
	unregister_widget('WP_Widget_Meta');
	wp_register_sidebar_widget('meta', __('Meta', 'chaostheory'), 'widget_sandbox_meta');
	unregister_widget('WP_Widget_Links');
	wp_register_sidebar_widget('links', __('Links', 'chaostheory'), 'widget_sandbox_links');
